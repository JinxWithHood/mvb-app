document.addEventListener('DOMContentLoaded', () => {
    ...
    function animateVehicleMarker(routeContainer, type, data) {
        ...
        function updatePosition() {
            ...
            // Apply color transition gradient if a transition is present
            const transitionStopIdx = data.stops.findIndex(s => s.transitionLine);
            if (transitionStopIdx !== -1 && transitionStopIdx < stopDoms.length) {
                const dotTrans = stopDoms[transitionStopIdx].querySelector('.stop-dot');
                const coordsTrans = getRelativeOffset(dotTrans, routeContainer);
                const transY = coordsTrans.top + dotTrans.offsetHeight / 2;
                
                const totalHeight = lastY - 10;
                const transHeight = transY - 10;
                const P = totalHeight > 0 ? (transHeight / totalHeight) * 100 : 0;
                
                const color1 = data.lineColor || 'var(--primary)';
                const color2 = data.transitionColor || 'var(--primary)';
                const gradientStr = `linear-gradient(to bottom, ${color1} 0%, ${color1} ${P}%, ${color2} ${P}%, ${color2} 100%)`;
                ...
                if (progressBar) {
                    ...
                    progressBar.style.boxShadow = targetTop >= transY 
                        ? `0 0 6px ${color2}` 
                        : `0 0 6px ${color1}`;
                }
            } else {
                const color = data.lineColor || 'var(--primary)';
                if (bgLine) {
                    bgLine.style.background = color;
                }
                if (progressBar) {
                    progressBar.style.background = color;
                    progressBar.style.boxShadow = `0 0 6px ${color}`;
                }
            }
