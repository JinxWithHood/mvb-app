document.addEventListener('DOMContentLoaded', () => {
    ...
    const MAP_URLS = {
        day: 'https://docs.google.com/gview?url=https://www.mvbnet.de/wp-content/uploads/2023/12/Liniennetzplan_Tagesverkehr.pdf&embedded=true',
        night: 'https://docs.google.com/gview?url=https://www.mvbnet.de/wp-content/uploads/2023/12/Liniennetzplan_Nachtverkehr.pdf&embedded=true'
    };
    ...
    function switchTab(targetTabId) {
        ...
        if (currentActivePanel) {
            ...
            setTimeout(() => {
                ...
                if (targetTabId === 'map' && mapFrame.src === 'about:blank') {
                    mapFrame.src = MAP_URLS.day;
                }
            }, 120);
        } else {
            targetPanel.hidden = false;
    ...
    [...headerNavLinks, ...tabButtonsMobile].forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (targetTab === 'alerts' || targetTab === 'map') {
    ...
    function renderDepartures(deps) {
        ...
        filtered.forEach((dep, idx) => {
            ...
            const stateText = dep.cancelled ? 'Ausfall' : (dep.delay > 0 ? `Verspätung +${dep.delay} Minuten` : 'pünktlich');
            ...
            const platformHtml = dep.platform 
                ? `<span class="departure-platform">Gleis ${dep.platform}</span>`
                : '';
    ...
    async function openJourneyDesktopPanel(dep) {
        ...
        const loadDetails = async (isQuiet = false) => {
            try {
                ...
                if (activeJourneyData && activeJourneyData.journey_id === dep.journey_id) {
                    renderStopsTimelineHTML(data, detailsRoute, detailsMessages);
                }
            } catch (err) {
        ...
        await loadDetails();
        journeyDetailInterval = setInterval(() => loadDetails(true), 10000);
    }
    ...
    async function openJourneyMobileModal(dep) {
        ...
        const loadDetails = async (isQuiet = false) => {
            ...
            try {
                ...
                renderStopsTimelineHTML(data, journeyRoute, journeyMessages);
            } catch (err) {
    ...
    function renderStopsTimelineHTML(data, routeContainer, messagesContainer) {
        ...
        timelineWrapper.className = 'journey-route animated';
        ...
        stops.forEach((stop, idx) => {
            ...
            const platformBadge = stop.platform 
                ? `<span class="stop-platform-badge">Gleis ${stop.platform}</span>`
                : '';
            ...
            conn.legs.forEach((leg, lIdx) => {
                ...
                if (isTransit) {
                    ...
                    if (leg.departure_delay > 0) {
                        delayHtml = `<span class="leg-delay positive" style="font-size:10px; margin-left:6px; color:var(--danger);">+${leg.departure_delay} Min</span>`;
                    }
                    ...
                    detailRow = `
                        <div class="connection-leg transit-leg" style="--leg-color: ${leg.lineColor || 'var(--primary)'}">
                            ...
                            <div class="leg-line-info">
                                ...
                                ${leg.platform ? `<span class="stop-platform-badge" style="margin-left:8px;">Gleis ${leg.platform}</span>` : ''}
                            </div>
                ...
                if (isShown) {
                    ...
                    try {
                        ...
                        data.stops.forEach(stop => {
                            ...
                            let delayText = '';
                            if (stop.delay > 0) delayText = ` <span style="color:var(--danger);">+${stop.delay}</span>`;
                            
                            row.innerHTML = `
                                <span style="color: ${stop.passed ? 'var(--text-secondary)' : 'var(--text-primary)'};">${nameText}</span>
                                <span style="color: var(--text-muted);">${stop.time}${delayText}</span>
                            `;
