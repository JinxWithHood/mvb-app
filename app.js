document.addEventListener('DOMContentLoaded', () => {
    ...
    const SITE_IS_OFFLINE = true;
    window.SITE_IS_OFFLINE = SITE_IS_OFFLINE;
    ...
    function hideLoadingScreen(force = false) {
        if (SITE_IS_OFFLINE && !force) return;
        const loadingScreen = document.getElementById('app-loading-screen');
        if (!loadingScreen) return;

        const elapsed = Date.now() - appLoadStartTime;
        if (!force && elapsed < MIN_LOADING_TIME) {
            setTimeout(() => hideLoadingScreen(true), MIN_LOADING_TIME - elapsed);
            return;
        }
    ...
    function renderOperator(operatorName, container) {
        ...
        if (opLower.includes('mvb') || opLower.includes('magdeburg')) {
            ...
            altText = 'MVB';
        } else if (opLower === 'db' || opLower.includes('deutsche bahn') || opLower.includes('db regio') || opLower.includes('db fernverkehr') || opLower.includes('dostgo') || opLower.includes('s-bahn') || opLower.includes('sbahn') || opLower.includes('db start')) {
            logoSrc = 'logo_db.png';
        ...
        // Wenn Logo erfasst ist, Bild anzeigen; falls nicht erfasst, bewusst weglassen ("Lass wenn das Logo nicht erfasst ist es weg")
        if (logoSrc) {
    ...
    function setupAutocomplete(inputEl, suggestionsEl, clearBtnEl, onSelect) {
        ...
        const renderSuggestions = (list) => {
            ...
            for (const station of list) {
                ...
                if ((!sid || !seenIds.has(sid)) && !seenNames.has(normName)) {
                    if (sid) seenIds.add(sid);
                    seenNames.add(normName);
                    uniqueList.push(station);
                }
            }
    ...
    async function fetchDepartures(silent = false) {
        ...
        try {
            ...
            // 24-Stunden-Archivierung im Hintergrund
            if (window.Archive24hEngine) {
                window.Archive24hEngine.saveDepartures(currentStation.id, currentStation.name, data);
            }
            ...
            // 24h Offline Fallback
            if (window.Archive24hEngine && currentStation && currentStation.id) {
                const cachedData = window.Archive24hEngine.getMergedDepartures(currentStation.id, currentStation.name, []);
                if (cachedData && cachedData.length > 0) {
                    departuresCache = cachedData;
                    departuresLoading.hidden = true;
                    renderDepartures(cachedData, silent);
                    if (isInitialLoad) {
                        isInitialLoad = false;
                        hideLoadingScreen();
                    }
    ...
    function renderDepartures(deps, silent = false) {
        ...
        const filtered = deps.filter(dep => {
            ...
            if (isNowModeActive) {
                ...
                if (actualDepDate - now < 0 || dep.isArchived) {
                    return false; // Im 'JETZT'-Modus keine abgefahrenen/archivierten Fahrten anzeigen!
                }
            }
            return true;
        });
        ...
        filtered.forEach((dep, idx) => {
            ...
            if (dep.isArchived) card.classList.add('is-archived');
            ...
            card.innerHTML = `
                ...
                <div class="departure-time">
                    ...
                    ${estimatedHtml}
                    <span class="departure-countdown" data-planned="${dep.time}" data-delay="${dep.delay || 0}" data-offset="${dep.day_offset}" ${dep.isArchived ? 'data-archived="true"' : ''}>${dep.isArchived ? 'Abgefahren' : ''}</span>
                    ${delayBadgeHtml}
                </div>
            `;
        ...
        const loadDetails = async (isQuiet = false) => {
            try {
                ...
                if (activeJourneyData && (activeJourneyData.card_journey_id || activeJourneyData.journey_id) === targetId) {
                    ...
                    if (window.Archive24hEngine) {
                        window.Archive24hEngine.saveJourney(targetId, data);
                    }
    ...
    async function openJourneyMobileModal(dep) {
        ...
        const loadDetails = async (isQuiet = false) => {
            ...
            try {
                ...
                if (window.Archive24hEngine) {
                    window.Archive24hEngine.saveJourney(mobTargetId, data);
                }
    ...
    function renderStopsTimelineHTML(data, routeContainer, messagesContainer, animate = true) {
        ...
        // 📄 Button: Offizielle Fahrt- & Verspätungsbescheinigung (für jede Fahrt verfügbar)
        const certBtn = document.createElement('button');
        certBtn.className = 'btn-generate-cert';
        const isProblematic = data.cancelled || (typeof data.delay === 'number' && data.delay > 0);
        certBtn.innerHTML = `<span>📄</span> <span>${isProblematic ? 'Offizielle Verspätungs- / Ausfallbescheinigung erstellen' : 'Fahrtbestätigung / Nachweis erstellen'}</span>`;
        certBtn.setAttribute('title', 'Dokument für Arbeitgeber, Schule oder Uni als PDF generieren');
        certBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            if (window.CertificateEngine) {
                window.CertificateEngine.openCertificateModal(data, activeJourneyData);
            }
        });
        routeContainer.appendChild(certBtn);
        ...
        conns.forEach((conn, idx) => {
            ...
            conn.legs.forEach((leg, lIdx) => {
                ...
                if (lIdx < conn.legs.length - 1) {
                    const trans = leg.transfer_to_next;
                    const isBroken = (trans && trans.is_broken) || (conn.missed_connection && (conn.broken_transfer_station === leg.destination || !conn.broken_transfer_station));
                    const bufferMins = trans ? trans.buffer_mins : null;
                    const transStation = (trans && trans.station) || leg.destination;

                    if (isBroken) {
                        timelineHtml += `
                            <div class="transfer-indicator transfer-warning" style="background:#fee2e2; border:1px solid #f87171; color:#991b1b; padding:8px 12px; border-radius:6px; margin:8px 0; font-weight:700; font-size:11px;">
                                ⚠️ Umsteigen in <strong>${escapeHtml(transStation)}</strong>: <span style="background:#dc2626; color:#ffffff; padding:1px 6px; border-radius:3px; margin-left:4px;">Anschluss kann vsl. nicht erreicht werden</span>
                                ${bufferMins !== null ? `<div style="font-size:10px; font-weight:600; color:#b91c1c; margin-top:3px;">Kalkulierter Puffer: ${bufferMins} Min.</div>` : ''}
                            </div>
                        `;
                    } else {
                        timelineHtml += `
                            <div class="transfer-indicator">
                                <span>🔄 Umsteigen in ${escapeHtml(transStation)} ${bufferMins !== null ? `(Puffer: ca. ${bufferMins} Min.)` : ''}</span>
                            </div>
                        `;
                    }
                }
            ...
            // Button für Verbindungsnachweis am Ende der Details
            timelineHtml += `
                <div style="margin-top:14px; border-top:1px dashed var(--card-border); padding-top:12px;">
                    <button type="button" class="btn-generate-conn-cert" style="width:100%; padding:9px 14px; background:rgba(1,142,74,0.08); border:1.5px solid #018e4a; color:#018e4a; border-radius:6px; font-size:12px; font-weight:700; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:6px;">
                        <span>📄</span> <span>Offiziellen Nachweis für Verbindung erstellen</span>
                    </button>
                </div>
            `;
            timelineHtml += `</div>`;
            ...
            let missedAlertHtml = '';
            if (conn.missed_connection) {
                missedAlertHtml = `
                    <div class="connection-missed-alert" style="background:#fef2f2; border:1px solid #ef4444; color:#b91c1c; font-size:12px; font-weight:800; padding:6px 12px; border-radius:6px; margin:8px 0; display:flex; align-items:center; gap:8px;">
                        <span>⚠️</span>
                        <span>Anschluss kann vsl. nicht erreicht werden</span>
                    </div>
                `;
            }
            ...
            card.innerHTML = `
                ...
                ${legsPreviewHtml}
                ${missedAlertHtml}
                <div class="connection-cert-banner" style="margin: 8px 0 4px 0;">
                    <button type="button" class="btn-generate-conn-cert connection-cert-action-btn" title="Offiziellen Nachweis für diese Verbindung erstellen">
                        <span>📄</span> <span>Offiziellen Nachweis für Verbindung erstellen</span>
                    </button>
                </div>
                <div class="connection-toggle-hint" style="font-size:11px; color:var(--text-secondary); text-align:center; padding:4px 0; cursor:pointer;">Klicke für Verbindungsdetails &amp; Zwischenhalte</div>
                ${timelineHtml}
            `;

            const handleConnCertClick = (e) => {
                e.stopPropagation();
                e.preventDefault();
                const fromName = (originStation && originStation.name) || (originInput && originInput.value.trim()) || (firstLeg ? firstLeg.origin : 'Start');
                const toName = (destStation && destStation.name) || (destInput && destInput.value.trim()) || (lastLeg ? lastLeg.destination : 'Ziel');
                if (window.CertificateEngine) {
                    window.CertificateEngine.openConnectionCertificateModal(conn, fromName, toName);
                } else {
                    console.error("CertificateEngine not loaded");
                }
            };

            card.querySelectorAll('.btn-generate-conn-cert').forEach(btn => {
                btn.addEventListener('click', handleConnCertClick);
            });
            ...
            const toggleDetails = () => {
                ...
                const promptText = card.querySelector('.connection-toggle-hint');
                if (promptText) {
                    promptText.textContent = isExpanded ? 'Klicke für Verbindungsdetails & Zwischenhalte' : 'Details ausblenden';
                }
            };

            card.addEventListener('click', (e) => {
                if (e.target.closest('.show-live-tracking-btn') || e.target.closest('.btn-generate-conn-cert')) return;
                toggleDetails();
            });
    ...
    setInterval(() => {
        if (!SITE_IS_OFFLINE && newsFetchedOnce) {
    ...
    document.addEventListener('visibilitychange', () => {
        if (!SITE_IS_OFFLINE && !document.hidden && newsFetchedOnce) {
            fetch(`${API_BASE}/news?_=${Date.now()}`)
                .then(r => r.ok ? r.json() : null)
                .then(latestNews => {
                    if (Array.isArray(latestNews) && latestNews.length > 0) {
    ...
    // If site is marked offline, do not connect to the server or dismiss the offline screen
    if (!SITE_IS_OFFLINE) {
        switchTab('home');
        setTimeout(() => {
            hideLoadingScreen();
    ...
    // Global helpers to preview or toggle offline mode
    window.previewJinxLoading = function(duration = 4000) {
        let screen = document.getElementById('app-loading-screen');
        if (!screen) {
            screen = document.createElement('div');
            screen.id = 'app-loading-screen';
            screen.className = 'loading-overlay';
            screen.innerHTML = `
                <div class="loading-container">
                    <div class="loading-flexity-wrapper">
                        <img src="assets/tram_mvb.png" alt="Flexity Magdeburg" class="loading-flexity-img">
                    </div>
                    <div class="jinx-glitch-wrapper">
                        <div class="loading-status-text jinx-glitch-text" id="loadingStatusText" data-text="DIE SEITE IST AKTUELL OFFLINE">DIE SEITE IST AKTUELL OFFLINE</div>
                    </div>
                </div>
            `;
        ...
        if (duration > 0) {
            setTimeout(() => {
                ...
                setTimeout(() => { if (screen && screen.parentNode) screen.parentNode.removeChild(screen); }, 500);
            }, duration);
        }
    };
    // Certificate Modal Bindings
    const btnPrintCert = document.getElementById('btnPrintCertificate');
    if (btnPrintCert) {
        btnPrintCert.addEventListener('click', () => {
            if (window.CertificateEngine) window.CertificateEngine.printCertificate();
        });
    }
    const btnSavePdfCert = document.getElementById('btnSavePdfCertificate');
    if (btnSavePdfCert) {
        btnSavePdfCert.addEventListener('click', () => {
            if (window.CertificateEngine) window.CertificateEngine.saveAsPdf();
        });
    }
    const btnOpenWin = document.getElementById('btnOpenPrintWindow');
    if (btnOpenWin) {
        btnOpenWin.addEventListener('click', () => {
            if (window.CertificateEngine) window.CertificateEngine.openPrintWindow();
        });
    }
    const btnCloseCert = document.getElementById('btnCloseCertificate');
    if (btnCloseCert) {
        btnCloseCert.addEventListener('click', () => {
            if (window.CertificateEngine) window.CertificateEngine.closeCertificateModal();
        });
    }
});
