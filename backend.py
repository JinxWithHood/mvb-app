        ...
        if client:
            try:
                if not origin.isdigit():
                    locs = client.locations(origin)
                    if locs:
                        origin = locs[0].id
                    else:
                        print(f"Origin location '{origin}' not resolved by HAFAS.")
            except Exception as e:
                print(f"Error resolving origin location '{origin}': {e}")
                
            try:
                if not destination.isdigit():
                    locs = client.locations(destination)
                    if locs:
                        destination = locs[0].id
                    else:
                        print(f"Destination location '{destination}' not resolved by HAFAS.")
            except Exception as e:
