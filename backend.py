...
# Custom OEBB Profile to support transit searches in other cities (like Munich)
class OEBBProfile(DBProfile):
    ...
    def parse_journeys_request(self, data):
        ...
        for jny in data.res.get('outConL', []):
            ...
            journeys.append(
                Journey(
                    jny.get('ctxRecon', ''), date=date, duration=self.parse_timedelta(
...
# Initialize HAFAS Clients
try:
    nasa_client = HafasClient(NASAProfile())
except Exception as e:
    print("Warning: Could not initialize NASA client:", e)
    nasa_client = None

try:
    oebb_client = HafasClient(OEBBProfile())
except Exception as e:
    print("Warning: Could not initialize OEBB client:", e)
    oebb_client = None

# Reference main client for backwards compatibility
client = nasa_client
...
def classify_line_type(line_name, mode_name=None):
    if not line_name:
        return "walk"
    ...
    if any(k == line_lower or line_lower.startswith(k + " ") or line_lower.startswith(k) for k in express_keywords):
        return "express"
        
    # Regional / S-Bahn: RE, RB, ERB, S, S-Bahn, etc.
    is_regional = any(line_lower.startswith(k) for k in ["re", "rb", "erb"]) or \
                  line_lower == "s" or \
                  line_lower.startswith("s ") or \
                  (line_lower.startswith("s") and line_lower[1:].isdigit()) or \
    ...
    # Tram: Tram, Str, or <= 15
    clean_num = line_name.replace(" ", "").replace("Str", "").replace("Tram", "").replace("Bus", "").replace("Linie", "").replace("Line", "").strip()
    is_num = clean_num.isdigit()
    
    is_tram = any(x in line_lower for x in ["str", "tram"]) or \
...
def search_station():
    ...
    try:
        ...
        # 1. Query NASA Client (primary for Magdeburg/Saxony-Anhalt)
        if nasa_client:
            try:
                ...
                print("NASA locations search failed:", ex)
                
        # 2. Query OEBB Client (primary for other cities/national)
        if oebb_client:
            try:
                ...
                print("OEBB locations search failed:", ex)
...
def get_departures():
    ...
    # Route dynamically: Use oebb_client for non-local (7+ digit ID) stations
...
def get_journey():
    ...
    if nasa_client:
        try:
            ...
            print(f"NASA JourneyDetails query failed for {journey_id}: {e}")
    ...
    if res is None and oebb_client:
        try:
            ...
            print(f"OEBB JourneyDetails query failed for {journey_id}: {e}")

    if res is None:
        print(f"All HAFAS clients failed for journey {journey_id}. Utilizing fallback generator.")
        return generate_fallback_journey(fallback_line, fallback_direction, fallback_time, fallback_delay, station_name)
        ...
        for msg in msgL:
            ...
            if remX is not None and remX < len(remL):
                ...
                if text:
                    ...
                    is_warning = any(x in text_lower for x in ["ausfall", "umleitung", "ersatzverkehr", "gesperrt", "störung", "verspätung"])
                    messages.append({
                        "text": text,
                        "warning": is_warning
                    })
        ...
        if nasa_client or oebb_client:
            try:
                if not origin.isdigit():
                    ...
                    if nasa_client:
                        try:
                            locs = nasa_client.locations(origin)
                            if locs:
                                resolved_origin = locs[0].id
                                resolved = True
                        except Exception:
                            pass
                    if not resolved and oebb_client:
                        try:
                            locs = oebb_client.locations(origin)
                            if locs:
                                resolved_origin = locs[0].id
                        except Exception:
                            pass
            except Exception as e:
                print(f"Error resolving origin location '{origin}': {e}")
            ...
            try:
                if not destination.isdigit():
                    ...
                    if nasa_client:
                        try:
                            locs = nasa_client.locations(destination)
                            if locs:
                                resolved_dest = locs[0].id
                                resolved = True
                        except Exception:
                            pass
                    if not resolved and oebb_client:
                        try:
                            locs = oebb_client.locations(destination)
                            if locs:
                                resolved_dest = locs[0].id
                        except Exception:
                            pass
            except Exception as e:
                print(f"Error resolving destination location '{destination}': {e}")

        # Choose the client dynamically
        active_client = nasa_client
        if resolved_origin and len(str(resolved_origin)) >= 7 and resolved_dest and len(str(resolved_dest)) >= 7:
            if oebb_client:
                active_client = oebb_client
        ...
        if active_client:
            ...
            for j in journeys:
                ...
                for leg in j.legs:
                    ...
                    # Day change check for leg departure and arrival
                    dep_day_offset = 0
                    arr_day_offset = 0
                    if leg.departure:
