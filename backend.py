...
def get_departures():
    ...
    if station_id and len(str(station_id)) >= 7:
    ...
    # Map Magdeburg local plazas to DB main station ID
    MAIN_STATION_MAP = {
        ...
        if active_client and departures is not None:
            ...
            for dep in departures:
                ...
                if delay is None and line_type in ["regional", "express"] and db_deps:
                    ...
                    prim_time = dep.dateTime.astimezone(local_tz) if dep.dateTime else None
                    for db_dep in db_deps:
                        ...
                        if cleaned_prim_line == cleaned_db_line or cleaned_prim_line in cleaned_db_line or cleaned_db_line in cleaned_prim_line:
                            db_time = db_dep.dateTime.astimezone(local_tz) if db_dep.dateTime else None
...
def get_journey():
    ...
    try:
        ...
        for s in stops_raw:
            ...
            stop_color = get_line_color(stop_line)
        ...
        if transition_info:
            ...
            if trans_idx != -1:
                line_after = clean_db_line_name(transition_info["newLine"])
                line_before = clean_db_line_name(fallback_line) if fallback_line else journey_line
                ...
                if line_before.lower().replace(" ", "") == line_after.lower().replace(" ", ""):
                    line_before = clean_db_line_name(journey_line)
                    ...
                    if line_before.lower().replace(" ", "") == line_after.lower().replace(" ", "") and fallback_line:
                        line_before = clean_db_line_name(fallback_line)

                # If still resolved to the same, look at the first stop's line name as a fallback
                if line_before.lower().replace(" ", "") == line_after.lower().replace(" ", ""):
                    first_stop_line = stops[0].get("line")
                    if first_stop_line and first_stop_line.lower().replace(" ", "") != line_after.lower().replace(" ", ""):
                        line_before = clean_db_line_name(first_stop_line)
