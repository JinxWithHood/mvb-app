...
def search_station():
    ...
    try:
        ...
        # 1.5. DB Client (national coverage for Berlin, Munich, etc.)
        if db_client:
            try:
                for loc in db_client.locations(query):
                    if loc.id and loc.id not in seen_ids:
                        results.append({"id": loc.id, "name": loc.name})
                        seen_ids.add(loc.id)
            except Exception as ex:
        ...
        # 2. OEBB Client (national / other cities fallback)
        if oebb_client:
            try:
...
def get_departures():
    ...
    for client_candidate in client_order:
        try:
            departures = client_candidate.departures(
                station=station_id,
                date=search_dt,
                duration=duration,
                max_trips=40
