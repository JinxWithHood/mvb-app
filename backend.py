...
def get_line_color(line_name: str, operator_name: str = None) -> str:
    ...
    # 1. Direct LINE_COLORS lookup first (matches '9', 'Str 9', 'N1', etc.)
...
def generate_fallback_journey(line: str, direction: str, start_time_str: str,
    ...
    for i, name in enumerate(stops_list):
        ...
        DEMAND_STOP_KEYWORDS = ['wasserwerk', 'zirkusmuseum', 'pallasweg', 'poststraße', 'beyendorf', 'salbke', 'sülzetal', 'hopfengarten']
        is_req = any(kw in name.lower() for kw in DEMAND_STOP_KEYWORDS)
        stops.append({
            "name": name,
            "time": time_str,
            "delay": stop_delay,
            ...
            "isRequestStop": is_req,
            "requestStopNote": "Halt bei Bedarf" if is_req else None
        })
...
def get_journey():
    ...
    try:
        ...
        for s in stops_raw:
            ...
            # Check request stop / Halt bei Bedarf flags & remarks
            stop_req_note = None
            ...
            for s_msg in stop_msg_list:
                ...
                if remX is not None and remX < len(remL_common):
                    ...
                    if txt_val:
                        ...
                        if any(kw in txt_lower for kw in ['bedarf', 'request', 'verlangen', 'bedarfshalt']):
                            is_req_stop = True
                            stop_req_note = txt_val
                            break

            # Magdeburg known request stops / night line stops fallback
            DEMAND_STOP_KEYWORDS = ['wasserwerk', 'zirkusmuseum', 'pallasweg', 'poststraße', 'beyendorf', 'salbke', 'sülzetal', 'hopfengarten']
            clean_name_lower = name.lower()
            if not is_req_stop and any(kw in clean_name_lower for kw in DEMAND_STOP_KEYWORDS):
                is_req_stop = True
        ...
        for msg in messages:
            txt = msg.get('text', '')
            txt_lower = txt.lower()
            if any(kw in txt_lower for kw in ['bedarf', 'verlangen', 'bedarfshalt']):
                for s_item in stops:
